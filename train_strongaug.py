...
def train(cfg, logger, pretrain = None ,checkpoint = None, output_dir= None, epoch =None):
    ...
    wandb.init(project = "deep learning", name = "hard label", resume = True)
    ...
    while iteration < stop_iter:
        for (images, labels), (u_imgs, _) in zip(supervised_loader, unsupervised_loader):
            ...
            if iteration % 1000 == 0:
                ...
                wlog.update({"iou": float(str(mean_iou)), "overall_acc": acc, "lr": float(str(optimizer.param_groups[0]['lr']))})
                results = results[:-2]
                logger.info(results)
